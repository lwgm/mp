...
(e.g. when we use "str" type `ups` variables or check their "validity").
        ...
        self.__writer.write( b"LIST UPS\n" )
        await self.__writer.drain()
        # result = self.__srv_handler.read_until( b"\n" )
        result = await self.__reader.readuntil()
        ...
        result = await self.__reader.readuntil( b"END LIST UPS\n" )
        ups_list = {}
...
available vars.
        ...
        self.__writer.write( ("LIST VAR %s\n" % ups).encode('ascii') )
        await self.__writer.drain()
        # result = self.__srv_handler.read_until( b"\n" )
        result = await self.__reader.readuntil()
        ...
        result     = await self.__reader.readuntil(("END LIST VAR %s\n" % ups).encode('ascii'))
    ...
    async def CheckUPSAvailable( self, ups="" ) :
...
The result is True (reachable) or False (unreachable)
        ...
        self.__writer.write( ("LIST CMD %s\n" % ups).encode('ascii') )
        await self.__writer.drain()
        # result = self.__srv_handler.read_until( b"\n" )
        result = await self.__reader.readuntil()
        ...
        await self.__reader.readuntil(("END LIST CMD %s\n" % ups).encode('ascii'))
        return True

    async def GetUPSCommands( self, ups="" ) :
...
of the command as value
        ...
        result = await self.__reader.readuntil( ("END LIST CMD %s\n" % ups).encode('ascii') )
        ...
        for current in result[:end_offset].split( b"\n" ) :
            ...
            try :
                ...
                self.__writer.write( ("GET CMDDESC %s %s\n" % ( ups, var )).encode('ascii') )
                await self.__writer.drain()
                # temp = self.__srv_handler.read_until( b"\n" )
                temp = await self.__reader.readuntil()
                if temp[:7] != b"CMDDESC" :
                    raise PyNUTError
                else :
...
The result is presented as a dictionary containing 'key->val' pairs
        ...
        result     = await self.__reader.readuntil( ("END LIST RW %s\n" % ups).encode('ascii') )
        offset     = len( ("VAR %s" % ups).encode('ascii') )
    ...
    async def SetRWVar( self, ups="", var="", value="" ):
...
rights to set it (maybe login/password).
        ...
        self.__writer.write( ("SET VAR %s %s %s\n" % ( ups, var, value )).encode('ascii') )
        await self.__writer.drain()
        # result = self.__srv_handler.read_until( b"\n" )
        result = await self.__reader.readuntil()
        if ( result == b"OK\n" ) :
    ...
    async def RunUPSCommand( self, ups="", command="" ) :
...
Returns OK on success or raises an error
        ...
        self.__writer.write( ("INSTCMD %s %s\n" % ( ups, command )).encode('ascii') )
        await self.__writer.drain()
        # result = self.__srv_handler.read_until( b"\n" )
        result = await self.__reader.readuntil()
        if ( result == b"OK\n" ) :
    ...
    async def DeviceLogin( self, ups="") :
...
(and backwards-compatible alias handling)
    ...
    async def help(self) :
        ...
        self.__writer.write( b"HELP\n" )
        await self.__writer.drain()
        # return self.__srv_handler.read_until( b"\n" )
        return await self.__reader.readuntil()
    ...
    async def ver(self) :
        ...
        self.__writer.write(b"VER\n" )
        await self.__writer.drain()
        # return self.__srv_handler.read_until( b"\n" )
        return await self.__reader.readuntil()

    async def ListClients( self, ups = None ) :
...
The result is a dictionary containing 'key->val' pairs of 'UPSName' and a list of clients
        ...
        if ups:
            ...
            self.__writer.write( ("LIST CLIENT %s\n" % ups).encode('ascii') )
            await self.__writer.drain()
        else:
            ...
            self.__writer.write( b"LIST CLIENT\n" )
            await self.__writer.drain()
        # result = self.__srv_handler.read_until( b"\n" )
        result = await self.__reader.readuntil()
        ...
        if ups :
            ...
            result = await self.__reader.readuntil( ("END LIST CLIENT %s\n" % ups).encode('ascii') )
        else:
            # Should not get here with current NUT:
            # result = self.__srv_handler.read_until( b"END LIST CLIENT\n" )
            result = await self.__reader.readuntil( b"END LIST CLIENT\n" )
        ups_list = {}
